...
ICMessageMapper::ICMessageMapper() : Mapper()
{
    ...
    addMapping(settings.value(COMMAND_CONFIG_TAG,DEFAULT_APP_TAG).toString(), "cmd");
    addMapping(settings.value(SID_CONFIG_TAG,DEFAULT_CMD_TAG).toString(), "sid");
    addMapping(settings.value(UID_CONFIG_TAG,DEFAULT_SID_TAG).toString(), "uid");
